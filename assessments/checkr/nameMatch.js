...
const splitName = nameStr => {
  ...
  storage.push(nameSplit.length);
  storage.push(nameSplit[0]);
  if (nameSplit.length === 2) {
    storage.push(nameSplit[1]);
    storage.push('');
  } else {
    storage.push(nameSplit[2]);
    storage.push(nameSplit[1]);
  }
...
const name_match = (namesArr, inputString) => {
  let [inputLength, inputFirst, inputLast, inputMiddle] = splitName(inputString);

  for (let name of namesArr) {
    let [nameLength, nameFirst, nameLast, nameMiddle] = splitName(name);

    if (nameLast === inputLast) {
      if (nameLength === 3 && inputLength === 3) {
        if (nameFirst === inputFirst) {
          if (nameMiddle === inputMiddle) {
            return true;
          }
          if (nameMiddle.length !== inputMiddle.length) {
            if (nameMiddle[0] === inputMiddle[0]) {
              return true;
            }
          }
        } else if (nameMiddle === inputFirst) {
          if (nameFirst[0] === inputMiddle || nameFirst === inputMiddle) {
            return true;
          }
        }
      } else {
        if (nameFirst === inputFirst || nameMiddle === inputFirst) {
          return true;
        }
      }
    }
